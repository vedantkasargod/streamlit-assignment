...
def load_apriori_rules():
    model_path = os.path.join(MODEL_DIR, 'apriori_model.pkl')
    with open(model_path, 'rb') as f:
        return pickle.load(f)
...
def prediction_fragment(inputs):
    ...
    if model_option == "Linear Regression (Predict Fare)":
        ...
        st.subheader("Get Association Rules using Apriori")
        load_apriori_rules()
        # You may display the association rules here if needed
